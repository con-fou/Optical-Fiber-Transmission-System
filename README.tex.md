...
|:--:| 
| *Figure 1: (a) Pulse-width modulated signal with rate of symbol transmission $25*10^9 \text{symbols/sec}$. (b) The corresponding eye diagram for pulse superposition within two $T_symb$ .* |
...
where:
	...
	$D$: the dispersion coefficient of the fiber
	$\beta _2$: 
	$z$: the distance of propagation
...
$\left\lfloor -\frac{N}{2} \right\rfloor ≤ k≤ \left\lfloor \frac{N}{2} \right\rfloor$, $N=2* \left\lfloor\frac{|D|\lambda ^2z}{2cT_s^2}\right\rfloor+1$
